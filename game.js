...
function playLaserSound() {
    ...
    osc.connect(gain);
    gain.connect(hpfNode); // Connect to start of the filter chain
...
function playExplosionSound() {
    ...
    // Custom sound structure (Noise buffer + Low sweep)
...
function playBackgroundMusic(filename, loop = true) {
    ...
    // Handle missing files gracefully without breaking execution
...
class Player {
    ...
    loseLife() {
        ...
        if (this.shieldActive) {
            ...
            playPickupSound(); // Play bounce/absorb noise
...
function startLevel(levelNumber) {
    ...
    // Play music for the newly loaded level
...
function updatePlayerPosition(player, frameRateFactor) {
    ...
    if (player.id === 'player1') {
        // Player 1 uses WASD (and Gamepad 0 or Touch Input)
...
function showTouchControls(visible) {
    if (visible) {
        ...
        if (numPlayers === 1) {
            // 1 Player: Use Right Joystick and Left Fire Button
...
function handleTouchStart(e) {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (numPlayers === 1) {
            // 1 Player layout: Player 1 controls mapped dynamically:
            // Right Joystick for movement, Left Fire Button for firing
            ...
            // 2 Players layout:
            // Player 1 Controls (left half of the screen: Left Joystick & Left Fire Button)
            if (touchPos.x < GAME_WIDTH / 2 && players[0] && players[0].lives > 0) {
            ...
            // Player 2 Controls (right half of the screen: Right Joystick & Right Fire Button)
...
function handleTouchMove(e) {
    ...
    for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (touchInput.player1.joystickActive && touch.identifier === touchInput.player1.touchId) {
            if (numPlayers === 1) {
                // Moving right side joystick
                updateJoystickHandlePosition(touchJoystickRightHandle, touchInput.player1, touchPos);
            } else {
                // Moving left side joystick
                updateJoystickHandlePosition(touchJoystickLeftHandle, touchInput.player1, touchPos);
            }
        }
        // Player 2 Joystick
